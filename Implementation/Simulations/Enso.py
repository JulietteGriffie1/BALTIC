...
x_train = np.zeros((6000, 1, 144, 64), dtype=np.float32)
for i in range(1, 3001):
    #Write the path to your data sets
    path = "Data sets\\STANDARD_SIMULATION\\"
    tmp = np.loadtxt(path + str(i) + ".csv", dtype=np.float32, delimiter=",")
    x_train[i-1] = tmp[:144, :64]
    x_train[i-1] = x_train[i-1]/x_train[i-1].max()

for i in range(3001, 6001):
    #write the path to your data sets if there are spread over multiple folders for data augmentation purposes
    path = "Data sets\\STANDARD_SIMULATION_mir\\"
    tmp = np.loadtxt(path + str(i-3000) + ".csv", dtype=np.float32, delimiter=",")
    x_train[i-1] = tmp[:144, :64]
    x_train[i-1] = x_train[i-1]/x_train[i-1].max()

x_test = np.zeros((3000, 1, 144, 64), dtype=np.float32)#x_test consists of the 
...
class VAE(nn.Module):
    ...
    def __init__(self, imgChannels=1, featureDim=(144/16)*(64/16)*256, zDim=2):
        #zdim=latent space
    ...
    def encoder(self, x):
        ...
        x = x.view(-1, int((144/16)*(64/16)*256))
    ...
    def decoder(self, z):
        ...
        x = x.view(-1, 256, int(144/16), int(64/16))
...
for epoch in range(num_epochs):
    print(epoch)
    for batch in range(1,int(1+6000/batch_size)):
...
for k in range(n):
    ...
    plot.imshow(x_test[k].reshape(144,64))
    ax=plot.subplot(2,n,k+1+n)
    plot.imshow(out_test[k].reshape(144,64))
