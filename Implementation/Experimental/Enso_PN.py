...
x_train = np.zeros((4972, 1, 64, 192), dtype=np.float32)
for i in range(1, 2487):
    path = "Data sets\\PN\\"
    tmp = np.loadtxt(path + str(i) + ".csv", dtype=np.float32, delimiter=",")
    x_train[i-1] = tmp[:64, :192]
    x_train[i-1] = x_train[i-1]/x_train[i-1].max()

for i in range(2487, 4973):
    path = "Data sets\\PN_mir\\"
    tmp = np.loadtxt(path + str(i-2486) + ".csv", dtype=np.float32, delimiter=",")
    x_train[i-1] = tmp[:64, :192]
    x_train[i-1] = x_train[i-1]/x_train[i-1].max()

x_test = np.zeros((2486, 1, 64, 192), dtype=np.float32)#x_test consists of the 
